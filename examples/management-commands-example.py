...
class EnergyCalculationCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not device_id:
            self.stdout.write(self.style.ERROR("Device ID richiesto"))
            return

        self.stdout.write(
            f"Calcolo energia per dispositivo {device_id} per {days} giorni..."
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Energia totale: {total_energy:.2f} kWh")
            )

            for day_data in daily_energy:
                self.stdout.write(f"  {day_data['date']}: {day_data['energy']:.2f} kWh")

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Errore nel calcolo: {str(e)}"))
...
class CERSetupCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("Setup CER di test completato!"))
...
class DocumentProcessingCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not file_path or not os.path.exists(file_path):
            self.stdout.write(self.style.ERROR("File path valido richiesto"))
            return
        ...
        try:
            ...
            if result["success"]:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'Errore nell\'elaborazione: {result.get("error")}'
                    )
                )
