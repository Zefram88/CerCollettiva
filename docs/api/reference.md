...
## Validatori Pydantic - Sistema di Validazione Centralizzato

### Panoramica
Il sistema CerCollettiva implementa un sistema di validazione input robusto utilizzando Pydantic per prevenire injection attacks e garantire l'integrità dei dati.

### Moduli Validatori

#### Energy Validators
**Percorso**: `energy/validators/`

##### Device Validators
- **DeviceCreateValidator**: Validazione creazione dispositivi
- **DeviceUpdateValidator**: Validazione aggiornamento dispositivi  
- **DeviceConfigurationCreateValidator**: Validazione configurazione dispositivi
- **DeviceConfigurationUpdateValidator**: Validazione aggiornamento configurazione

##### Measurement Validators
- **MeasurementCreateValidator**: Validazione creazione misurazioni
- **MeasurementUpdateValidator**: Validazione aggiornamento misurazioni
- **DeviceMeasurementDetailValidator**: Validazione dettagli misurazione per fase

#### Document Validators
**Percorso**: `documents/validators/`

##### Document Validators
- **DocumentCreateValidator**: Validazione creazione documenti
- **DocumentUpdateValidator**: Validazione aggiornamento documenti
- **DocumentAccessValidator**: Validazione accessi documenti
- **DocumentFileValidator**: Validazione file documenti
- **DocumentSearchValidator**: Validazione ricerca documenti

### Esempi di Utilizzo

#### Validazione Dispositivo
```python
from energy.validators import DeviceCreateValidator
...
validator = DeviceCreateValidator(**data)
print(validator.name)  # 'Test Device'
```

#### Validazione Documento
```python
from energy.validators import DocumentCreateValidator

# Dati validi
data = {
    'type': 'BILL',
    'notes': 'Bolletta energia',
    'plant_id': 1,
    'gdpr_consent': True
}

validator = DocumentCreateValidator(**data)
print(validator.type)  # 'BILL'
```
...
```python
# ❌ Bloccato
validator = DeviceCreateValidator(
    name='<script>alert("xss")</script>',
    device_type='SHELLY_PRO_3EM',
    serial_number='ABC123',
    installation_date=date(2024, 1, 1)
)
# ValidationError: Nome contiene caratteri non validi
```
...
validator = DeviceCreateValidator(
    ...
    device_type='SHELLY_PRO_3EM',
    serial_number='ABC123',
    description="Test'; DROP TABLE devices; --",
    installation_date=date(2024, 1, 1)
)
# ValidationError: Descrizione contiene caratteri non validi
```

#### Validazione GDPR
```python
# ❌ Bloccato - GDPR richiesto per ID_DOC
validator = DocumentCreateValidator(
    type='ID_DOC',
    notes='Documento identità',
    plant_id=1,
    gdpr_consent=False
)
# ValidationError: Consenso GDPR obbligatorio per questo tipo di documento
```
