...
class DocumentCreateValidator(BaseModel):
    ...
    type: Literal[
        'BILL', 'SYSTEM_CERT', 'GSE_DOC', 'PANELS_PHOTO', 
        'INVERTER_PHOTO', 'PANELS_LIST', 'ID_DOC', 'GAUDI', 'OTHER'
    ...
    data_classification: Literal['PUBLIC', 'INTERNAL', 'CONFIDENTIAL', 'PERSONAL'] = Field(
        'INTERNAL', description="Classificazione dati"
    )
    ...
    def validate_type(cls, v):
        ...
        if v not in allowed_types:
            raise ValueError(f'Tipo documento non valido. Consentiti: {", ".join(allowed_types)}')
        return v
    ...
    def validate_data_classification(cls, v):
        ...
        if v not in allowed_classifications:
            raise ValueError(f'Classificazione dati non valida. Consentite: {", ".join(allowed_classifications)}')
        return v
    ...
    def validate_retention_date(cls, v, values):
        ...
        if v is not None:
            ...
            if v > max_future_date:
                raise ValueError('Data di conservazione non può essere oltre 50 anni nel futuro')
...
class DocumentUpdateValidator(BaseModel):
    ...
    data_classification: Optional[Literal['PUBLIC', 'INTERNAL', 'CONFIDENTIAL', 'PERSONAL']] = None
    ...
    def validate_processing_status(cls, v):
        if v is not None:
            ...
            if v not in allowed_statuses:
                raise ValueError(f'Stato elaborazione non valido. Consentiti: {", ".join(allowed_statuses)}')
        return v
...
class DocumentFileValidator(BaseModel):
    ...
    size: int = Field(..., ge=1, le=10485760, description="Dimensione file in bytes (max 10MB)")
    ...
    def validate_content_type(cls, v):
        ...
        if v not in allowed_types:
            raise ValueError(f'Tipo contenuto non consentito. Consentiti: {", ".join(allowed_types)}')
...
class DocumentSearchValidator(BaseModel):
    ...
    def validate_query(cls, v):
        ...
        if v is not None:
            ...
            # Prevenire SQL injection
            if any(pattern in v.lower() for pattern in [';', '--', '/*', '*/', 'union', 'select', 'insert', 'update', 'delete']):
                raise ValueError('Query contiene caratteri non validi')
    ...
    def validate_date_to(cls, v, values):
        ...
        if v is not None:
            ...
            if date_from and v < date_from:
                raise ValueError('Data di fine non può essere anteriore alla data di inizio')
