...
class DeviceMeasurementModelTest(TransactionTestCase):
    ...
    def test_measurement_str_method(self):
        ...
        measurement = DeviceMeasurement.objects.create(**self.measurement_data)
        expected = (
            f"{self.device.device_id} - {measurement.measurement_type} - "
            f"{measurement.timestamp:%Y-%m-%d %H:%M:%S}"
        )
    ...
    def test_measurement_timestamp(self):
        ...
        DeviceMeasurement.objects.create(**m1_data)

        m2_data = self.measurement_data.copy()
        m2_data["timestamp"] = time2
        m2 = DeviceMeasurement.objects.create(**m2_data)

        measurements = DeviceMeasurement.objects.filter(device=self.device).order_by(
            "-timestamp"
        )
    ...
    def test_measurement_energy_accumulation(self):
        ...
        total = DeviceMeasurement.objects.filter(device=self.device).aggregate(
...
class MQTTBrokerModelTest(TransactionTestCase):
    ...
    def test_broker_active_status(self):
        ...
        MQTTBroker.objects.create(**broker2_data)
