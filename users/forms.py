...
class MinimalRegistrationForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["password2"].help_text = _(
            "Inserisci la stessa password per conferma"
        )
    ...
    def clean_email(self):
        ...
        if email and CustomUser.objects.filter(email=email).exists():
            raise forms.ValidationError(
                _("Un utente con questo indirizzo email esiste già.")
            )
        return email
...
class UserRegistrationForm(UserCreationForm):
    ...
    username = forms.CharField(
        label=_("Username"),
        help_text=_(
            "Richiesto. 150 caratteri o meno. Solo lettere, numeri e @/./+/-/_"
        ),
        widget=forms.TextInput(
    ...
    address = forms.CharField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"class": "form-control", "autocomplete": "street-address"}
        ),
    )
    ...
    class Meta:
        ...
        fields = [
            "first_name",
            "last_name",  # Prima nome e cognome
            "username",
            "email",  # Poi le credenziali di accesso
            "legal_type",
            "profit_type",  # Poi i tipi
            "legal_name",
            "address",
            "phone",  # Dati personaliS
            "fiscal_code",
            "vat_number",
            "pec",
            "sdi_code",  # Dati fiscali
            "registration_number",
            "statute_date",  # Dati associazione
            "religious_entity_code",  # Dati enti religiosi
            "password1",
            "password2",  # Password alla fine
            "privacy_policy",
            "data_processing",  # Consensi GDPR
        ]
    ...
    def clean_privacy_policy(self):
        ...
        if not privacy_policy:
            raise forms.ValidationError(
                _("Devi accettare la privacy policy per registrarti.")
            )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "last_name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": _("Cognome")}
            ),
