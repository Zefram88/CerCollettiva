...
class DeviceTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "vendor", "model", "description", "is_active")}),
        (
            "Misure Supportate",
            {
                "fields": (
...
class DeviceMeasurementAdmin(admin.ModelAdmin):
    ...
    def power_display(self, obj):
        ...
        return format_html('<span style="color: {};">{:.1f} W</span>', color, obj.power)
...
class MQTTBrokerAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Connection Settings", {"fields": ("host", "port", "username", "password")}),
        ("Security", {"fields": ("use_tls", "verify_cert", "ca_cert")}),
        (
            "Additional Info",
