...
from core.validators import BaseValidator, ValidationMixin, APIValidationMixin
...
class EnergyDataValidator(BaseValidator):
    ...
    def validate_device_id(self, value, cleaned_data=None):
        ...
        if not re.match(r'^[A-Za-z0-9_-]+$', value):
            raise ValidationError(_("L'ID dispositivo può contenere solo lettere, numeri, underscore e trattini"))
    ...
    def validate_mqtt_topic(self, value, cleaned_data=None):
        ...
        if not re.match(r'^/[A-Za-z0-9_/]+$', value):
            raise ValidationError(_("Il template MQTT può contenere solo lettere, numeri, underscore e slash"))
    ...
    def validate_phase_data(self, value, cleaned_data=None):
        ...
        for phase, data in value.items():
            if not isinstance(data, dict):
                raise ValidationError(_("I dati delle fasi devono essere un dizionario"))
            
            for field in required_fields:
                if field not in data:
                    raise ValidationError(_(f"Campo '{field}' mancante per la fase '{phase}'"))
                
                if data[field] is None:
                    raise ValidationError(_(f"Campo '{field}' non può essere nullo per la fase '{phase}'"))
    ...
    def validate_measurement_timestamp(self, value, cleaned_data=None):
        ...
        if value < one_year_ago:
            raise ValidationError(_("Il timestamp non può essere più vecchio di 1 anno"))
    ...
    def validate_date_range(self, start_date, end_date, cleaned_data=None):
        ...
        if end_date <= start_date:
            raise ValidationError(_("La data di fine deve essere successiva alla data di inizio"))
    ...
    def validate_period_appropriateness(self, period, start_date, end_date, cleaned_data=None):
        ...
        if period in period_limits:
            if time_diff > period_limits[period]:
                raise ValidationError(_(f"Il periodo {period} non è appropriato per range superiori a {period_limits[period].days} giorni"))
    ...
    def validate_measurement_quality(self, value, cleaned_data=None):
        ...
        if value not in valid_qualities:
            raise ValidationError(_(f"Qualità non valida. Valori consentiti: {', '.join(valid_qualities)}"))
    ...
    def validate_measurement_type(self, value, cleaned_data=None):
        ...
        valid_types = ['POWER', 'ENERGY', 'VOLTAGE', 'CURRENT', 'FREQUENCY', 'POWER_FACTOR']
        if value not in valid_types:
            raise ValidationError(_(f"Tipo misurazione non valido. Valori consentiti: {', '.join(valid_types)}"))
...
class DeviceDataValidator(EnergyDataValidator):
    ...
    def validate(self, data, cleaned_data=None):
        ...
        if 'device_id' in data:
            validated_data['device_id'] = self.validate_device_id(data['device_id'], cleaned_data)
        
        if 'mqtt_topic_template' in data:
            validated_data['mqtt_topic_template'] = self.validate_mqtt_topic(data['mqtt_topic_template'], cleaned_data)
        
        if 'device_config' in data:
            validated_data['device_config'] = self.validate_device_config(data['device_config'], cleaned_data)
...
class MeasurementDataValidator(EnergyDataValidator):
    ...
    def validate(self, data, cleaned_data=None):
        ...
        if 'timestamp' in data:
            validated_data['timestamp'] = self.validate_measurement_timestamp(data['timestamp'], cleaned_data)
        
        if 'phase_data' in data:
            validated_data['phase_data'] = self.validate_phase_data(data['phase_data'], cleaned_data)
        
        if 'quality' in data:
            validated_data['quality'] = self.validate_measurement_quality(data['quality'], cleaned_data)
        
        if 'measurement_type' in data:
            validated_data['measurement_type'] = self.validate_measurement_type(data['measurement_type'], cleaned_data)
            ...
            for field in numeric_fields:
                if field in payload:
                    try:
                        ...
                        raise ValidationError(_(f"Campo '{field}' deve essere numerico"))
        
        # Validazione topic
        if 'topic' in data:
            validated_data['topic'] = self.validate_mqtt_topic(data['topic'], cleaned_data)
...
class APIRequestValidator(EnergyDataValidator):
    ...
    def validate(self, data, cleaned_data=None):
        ...
        if 'plant' in data and 'user' in data:
            validated_data['plant'] = self.validate_plant_ownership(data['plant'], data['user'], cleaned_data)
...
class EnergyValidationMixin(ValidationMixin):
    ...
    def validate_energy_data(self, data, validator_type='measurement'):
        ...
        try:
            return validator.validate(data, self.cleaned_data if hasattr(self, 'cleaned_data') else None)
