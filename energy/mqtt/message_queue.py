    ...
    def enqueue(self, message: QueuedMessage, timeout: Optional[float] = None) -> bool:
        ...
        try:
            with self._not_full:
                ...
                if timeout is None:
                    ...
                    while len(self._queue) >= self._max_size:
                        ...
                        if remaining <= 0:
                            ...
                            logger.warning(f"Queue full, dropping message: {message.event_type}")
                            return False
                ...
                logger.debug(f"Message enqueued: {message.event_type} (queue size: {len(self._queue)})")
                return True
    ...
    def dequeue(self, timeout: Optional[float] = None) -> Optional[QueuedMessage]:
        ...
        try:
            with self._not_empty:
                ...
                logger.debug(f"Message dequeued: {message.event_type} (queue size: {len(self._queue)})")
                return message
    ...
    def remove_messages_by_type(self, event_type: str) -> int:
        ...
        try:
            with self._lock:
                ...
                self._queue = deque([msg for msg in self._queue if msg.event_type != event_type], 
                                  maxlen=self._max_size)
                removed_count = initial_size - len(self._queue)
                
                if removed_count > 0:
                    self._not_full.notify_all()
                    logger.info(f"Removed {removed_count} messages of type: {event_type}")
