...
class EnergyMQTTClient:
    ...
    def _setup_client(self) -> None:
        ...
        try:
            ...
            # LWT
            self._client.will_set("CerCollettiva/status", "offline", qos=1, retain=True)
    ...
    def _process_message(self, msg):
        ...
        try:
            ...
            if (
                cache_key in last_refresh
                and current_time - last_refresh[cache_key] < 60
            ):
        ...
        if self._client:
            try:
                self._client.publish(
                    "CerCollettiva/status", "offline", qos=1, retain=True
                )
    ...
    def _reconnect_with_backoff(self):
        def reconnect_thread():
            while not self.check_connection():  # usa il nuovo metodo rinominato
                try:
                    logger.info(
                        f"Tentativo di riconnessione tra {self._retry_delay}s..."
                    )
                    time.sleep(self._retry_delay)

                    if (
                        self._client.reconnect() == 0
                    ):  # verifica il risultato della riconnessione
