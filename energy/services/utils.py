...
def calculate_period_aggregates(
    ...
    while current_start < end_date:
        ...
        measurements = DeviceMeasurement.objects.filter(
            device_id=device_id, timestamp__gte=current_start, timestamp__lt=current_end
        )

        if measurements.exists():
            agg = measurements.aggregate(
                avg_power=Avg("power"), peak_power=Max("power"), min_power=Min("power")
            )
...
def get_latest_measurement(device_id: str) -> Optional[DeviceMeasurement]:
    ...
    """
    try:
        return DeviceMeasurement.objects.filter(device_id=device_id).latest("timestamp")
