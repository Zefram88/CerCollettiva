...
from ..models import DeviceMeasurement, DeviceMeasurementDetail, DeviceConfiguration
...
class MeasurementService:
    ...
    def process_power_measurement(self, context: MessageContext) -> bool:
        ...
        try:
            ...
            success = self._save_power_measurement(device_config, measurement_data, context)
    ...
    def _find_device_config(self, context: MessageContext) -> Optional[DeviceConfiguration]:
        """
        ...
        try:
            ...
            # Cerca per device_id
            device_config = DeviceConfiguration.objects.select_related('plant').filter(
    ...
    def _find_device_by_topic_pattern(self, topic: str) -> Optional[DeviceConfiguration]:
        """
        Trova la configurazione del dispositivo basata sul pattern del topic
        """
        try:
            # Cerca tra tutti i dispositivi attivi
            devices = DeviceConfiguration.objects.filter(is_active=True).select_related('plant')
    ...
    def _extract_phase_data(self, payload: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
        """
        ...
        for phase in phases:
            phase_fields = [f'{phase}_voltage', f'{phase}_current', f'{phase}_act_power']
    ...
    def _calculate_energy_delta(self, device_id: str, current_energy: float) -> Optional[float]:
        """
        ...
        try:
            ...
            if last_energy is not None:
                ...
                logger.info(f"First energy reading for device {device_id}: {current_energy} Wh")
                return None
    ...
    def _save_power_measurement(self, device_config: DeviceConfiguration, 
        ...
        try:
            with transaction.atomic():
                ...
                if measurement_data['phase_data']:
                    self._save_phase_details(measurement, measurement_data['phase_data'])
    ...
    def _save_energy_measurement(self, device_config: DeviceConfiguration, 
                                energy_delta: float, 
                                context: MessageContext) -> bool:
        """
    ...
    def _save_phase_details(self, measurement: DeviceMeasurement, 
    ...
    def _is_duplicate_message(self, context: MessageContext, 
                             device_config: DeviceConfiguration) -> bool:
        """
    ...
    def _cache_processed_message(self, context: MessageContext, 
