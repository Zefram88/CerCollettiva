...
def debug_device_status(request):
    devices = DeviceConfiguration.objects.filter(plant__name="Bernardi Srl").values(
        "id", "device_id", "is_active", "plant__name"
    )
...
def debug_mqtt_config(request):
    ...
    return JsonResponse(
        {
            "active_configs": list(
                configs.values("device_id", "mqtt_topic_template", "is_active")
            ),
