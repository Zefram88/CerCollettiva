...
def mqtt_settings(request):
    ...
    for log in recent_logs:
        ...
        if log.operation == "CONNECT":
            log.message = "Connessione al broker MQTT " + (
                "riuscita" if log.status else "fallita"
            )
            ...
            log.message = (
                log.details.get("message", "Nessun dettaglio disponibile")
                if log.details
                else "Nessun dettaglio disponibile"
            )
    ...
    context = {
        ...
        "stats": {
            "total_connections": MQTTAuditLog.objects.filter(
                operation="CONNECT", status=True
            ).count(),
            "failed_connections": MQTTAuditLog.objects.filter(
                operation="CONNECT", status=False
            ).count(),
...
def save_mqtt_settings(request):
    try:
        broker = (
            MQTTBroker.objects.get(id=request.POST.get("id"))
        ...
        messages.error(
            request, f"Errore nel salvataggio della configurazione: {str(e)}"
        )
