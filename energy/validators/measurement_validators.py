...
class MeasurementCreateValidator(BaseModel):
    ...
    measurement_type: Literal[
        'DRAWN_POWER', 'DRAWN_ENERGY', 'INJECTED_POWER', 
        'INJECTED_ENERGY', 'PRODUCTION_POWER', 'PRODUCTION_ENERGY'
    ...
    power_factor: Optional[float] = Field(None, ge=-1, le=1, description="Fattore di potenza")
    raw_data: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Dati grezzi JSON")
    ...
    def validate_raw_data(cls, v):
        ...
        if v is not None:
            ...
            for key, value in v.items():
                if isinstance(key, str) and any(char in key for char in ['<', '>', 'script', 'javascript:']):
                    raise ValueError('Chiave dati grezzi contiene caratteri non validi')
                
                if isinstance(value, str) and any(char in value for char in ['<script', 'javascript:', 'data:']):
...
class MeasurementUpdateValidator(BaseModel):
    ...
    measurement_type: Optional[Literal[
        'DRAWN_POWER', 'DRAWN_ENERGY', 'INJECTED_POWER', 
        'INJECTED_ENERGY', 'PRODUCTION_POWER', 'PRODUCTION_ENERGY'
    ]] = None
...
class DeviceMeasurementDetailValidator(BaseModel):
    ...
    power: float = Field(..., ge=-1000000, le=1000000, description="Potenza di fase in W")
    power_factor: Optional[float] = Field(None, ge=-1, le=1, description="Fattore di potenza di fase")
