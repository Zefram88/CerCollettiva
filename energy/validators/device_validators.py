...
class DeviceCreateValidator(BaseModel):
    ...
    name: str = Field(
        ..., min_length=3, max_length=100, description="Nome del dispositivo"
    )
    device_type: str = Field(
        ..., min_length=2, max_length=50, description="Tipo dispositivo"
    )
    serial_number: str = Field(
        ..., min_length=1, max_length=100, description="Numero seriale"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Descrizione opzionale"
    )
    installation_date: date = Field(..., description="Data di installazione")
    is_active: bool = Field(True, description="Dispositivo attivo")
    mqtt_topic_override: Optional[str] = Field(
        None, max_length=255, description="Override topic MQTT"
    )
...
class DeviceConfigurationCreateValidator(BaseModel):
    ...
    device_id: str = Field(
        ..., min_length=1, max_length=100, description="ID dispositivo"
    )
    ...
    mqtt_topic_template: Optional[str] = Field(
        None, max_length=255, description="Template topic MQTT"
    )
    firmware_version: Optional[str] = Field(
        None, max_length=50, description="Versione firmware"
    )
    is_active: bool = Field(True, description="Dispositivo attivo")
    ...
    def validate_model(cls, v):
        ...
        if not re.match(r"^[A-Za-z0-9_-]+$", v):
            raise ValueError(
                "Modello può contenere solo lettere, numeri, underscore e trattini"
            )
    ...
    def validate_firmware_version(cls, v):
        ...
        if v is not None:
            ...
            if not re.match(r"^\d+\.\d+(\.\d+)?$", v.strip()):
                raise ValueError(
                    "Formato versione firmware non valido (es. 1.2.3 o 1.2)"
                )
