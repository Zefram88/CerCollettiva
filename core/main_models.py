...
class CERDistributionConfiguration(models.Model):
    ...
    def get_distribution_breakdown(self, total_amount):
        ...
        return {
            "producers": round(
                total_amount * (self.producer_percentage / Decimal("100")), 2
            ),
            "consumers": round(
                total_amount * (self.consumer_percentage / Decimal("100")), 2
            ),
            "management": round(
                total_amount * (self.management_percentage / Decimal("100")), 2
            ),
            "investment_fund": round(
                total_amount * (self.investment_fund_percentage / Decimal("100")), 2
            ),
            "solidarity_fund": round(
                total_amount * (self.solidarity_fund_percentage / Decimal("100")), 2
            ),
            "total": total_amount,
        }
...
class GSEIncomeTracking(models.Model):
    ...
    payment_type = models.CharField(
        "Tipo Pagamento", max_length=20, choices=PAYMENT_TYPES
    )
    reference_month = models.DateField(
        "Mese di Riferimento", help_text="Mese a cui si riferisce il pagamento"
    )
    reference_year = models.IntegerField(
        "Anno di Riferimento", help_text="Anno a cui si riferisce il pagamento"
    )
    ...
    expected_payment_date = models.DateField(
        "Data Attesa Pagamento", null=True, blank=True
    )
    actual_payment_date = models.DateField(
        "Data Pagamento Effettivo", null=True, blank=True
    )
    payment_status = models.CharField(
        "Stato Pagamento", max_length=20, choices=PAYMENT_STATUS, default="EXPECTED"
    )
    ...
    notes = models.TextField(
        "Note", blank=True, help_text="Note aggiuntive sul pagamento"
    )
...
class CERMembership(models.Model):
    ...
    cer_configuration = models.ForeignKey(
        CERConfiguration, on_delete=models.CASCADE, related_name="memberships"
    )
    role = models.CharField(
        "Ruolo", max_length=20, choices=ROLE_CHOICES, default="MEMBER"
    )
    ...
    joined_date = models.DateField("Data di adesione", blank=True, null=True)
    share_percentage = models.DecimalField(
        "Percentuale di partecipazione", max_digits=5, decimal_places=2, default=0.00
    )
    ...
    def clean(self):
        ...
        if self.share_percentage > 100:
            raise ValidationError(
                {"share_percentage": "La percentuale non può superare il 100%"}
            )
        if self.share_percentage < 0:
            raise ValidationError(
                {"share_percentage": "La percentuale non può essere negativa"}
            )
    ...
    gse_practice = models.FileField(
        "Pratica GSE", upload_to="cer/gse/%Y/%m/%d/", null=True, blank=True
    )
    panels_photo = models.FileField(
        "Foto pannelli", upload_to="cer/panels/%Y/%m/%d/", null=True, blank=True
    )
    inverter_photo = models.FileField(
        "Foto inverter", upload_to="cer/inverter/%Y/%m/%d/", null=True, blank=True
    )
    panels_serial_list = models.TextField(
        "Lista seriali pannelli", blank=True, null=True
    )
    ...
    def create_membership_card(self):
        ...
        card = MembershipCard(
            membership=self, expiry_date=timezone.now() + timedelta(days=365)  # 1 anno
        )
...
class MembershipCard(models.Model):
    ...
    membership = models.OneToOneField(
        CERMembership, on_delete=models.CASCADE, related_name="card"
    )
    ...
    issue_date = models.DateTimeField("Data Emissione", auto_now_add=True)
    expiry_date = models.DateTimeField(
        "Data Scadenza", help_text="Data di scadenza della tessera"
    )
    is_active = models.BooleanField("Attiva", default=True)

    # Campi per gestione quote associative
    membership_fee_paid = models.BooleanField("Quota Pagata", default=False)
    fee_payment_date = models.DateTimeField(
        "Data Pagamento Quota", null=True, blank=True
    )
...
class MemberRegistry(models.Model):
    ...
    cer_configuration = models.ForeignKey(
        CERConfiguration, on_delete=models.CASCADE, related_name="member_registry"
    )
    progressive_number = models.PositiveIntegerField("Numero Progressivo")
    membership = models.ForeignKey(
        CERMembership, on_delete=models.CASCADE, related_name="registry_entry"
    )
    registration_date = models.DateTimeField("Data Registrazione", auto_now_add=True)
    notes = models.TextField(
        "Note", blank=True, help_text="Note aggiuntive sulla registrazione"
    )
    ...
    def register_member(cls, membership):
        ...
        last_entry = (
            cls.objects.filter(cer_configuration=membership.cer_configuration)
            .order_by("-progressive_number")
            .first()
        )
...
class Plant(models.Model):
    ...
    code = models.CharField(
        "Codice impianto", max_length=50, unique=True, default="PLANT001"
    )
    pod_code = models.CharField(
        "Codice POD", max_length=50, unique=True, blank=True, null=True
    )
    type = models.CharField(
        "Tipologia", max_length=20, choices=PLANT_TYPES, default="PRODUCER"
    )
    plant_type = models.CharField(
        "Tipologia", max_length=20, choices=PLANT_TYPES, blank=True, null=True
    )
    ...
    power_kw = models.DecimalField(
        "Potenza (kW)", max_digits=10, decimal_places=2, default=0.00
    )
    nominal_power = models.FloatField("Potenza nominale (kW)", blank=True, null=True)
    ...
    # Dati Gaudì
    gaudi_request_code = models.CharField(
        "Codice richiesta Gaudì", max_length=50, blank=True, null=True
    )
    censimp_code = models.CharField(
        "Codice CENSIMP", max_length=50, blank=True, null=True
    )

    # Campi aggiuntivi per compatibilità con i test
    pod = models.CharField("POD", max_length=50, blank=True, null=True)
    commissioning_date = models.DateField(
        "Data di messa in servizio", blank=True, null=True
    )
    ...
    gaudi_verified = models.BooleanField(
        "Verificato Gaudì",
        default=False,
        help_text="Indica se i dati sono stati verificati con attestazione Gaudì",
    )
    gaudi_verification_date = models.DateTimeField(
        "Data verifica Gaudì",
        null=True,
        blank=True,
    ...
    gaudi_upload_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Data caricamento GAUDÌ"
    )
    ...
    def verify_gaudi(
        ...
        try:
            ...
            logger.error(
                f"Errore durante la verifica Gaudì dell'impianto {self.id}: {str(e)}"
            )
    ...
    def has_valid_gaudi(self):
        return bool(
            self.gaudi_verified
            and self.gaudi_verification_date
            and self.gaudi_request_code
        )
    ...
    def __str__(self):
        code_display = self.code or self.pod_code or "N/A"
    ...
    def test_mqtt_connection(self):
        ...
        try:
            ...
            if self.mqtt_username or (
                hasattr(settings, "MQTT_USERNAME") and settings.MQTT_USERNAME
            ):
    ...
    def get_energy_today(self):
        ...
        return (
            self.mqtt_data.filter(
                variable_type__contains="ENERGY", timestamp__date=today
            )
            .order_by("-timestamp")
            .first()
        )
    ...
    def get_current_power(self):
        ...
        return (
            self.mqtt_data.filter(variable_type__contains="POWER")
            .order_by("-timestamp")
            .first()
        )
    ...
    def get_max_power_today(self):
        ...
        return (
            self.mqtt_data.filter(
                variable_type__contains="POWER", timestamp__date=today
            ).aggregate(max_power=models.Max("value"))["max_power"]
            or 0
        )
    ...
    def get_daily_energy_data(self, days=7):
        ...
        return (
            self.mqtt_data.filter(
                variable_type__contains="ENERGY", timestamp__date__gte=start_date
            )
            .annotate(date=models.functions.TruncDate("timestamp"))
            .values("date")
            .annotate(energy=models.Max("value"))
            .order_by("date")
        )
    ...
    def validate_gaudi_data(self):
        ...
        if not self.gaudi_request_code:
            validation_errors["gaudi_request_code"] = _(
                "Il codice richiesta è obbligatorio per attestati verificati"
            )

        if not self.censimp_code:
            validation_errors["censimp_code"] = _(
                "Il codice CENSIMP è obbligatorio per attestati verificati"
            )

        if not self.validation_date:
            validation_errors["validation_date"] = _(
                "La data di validazione è obbligatoria per attestati verificati"
            )
        elif self.validation_date > timezone.now():
            validation_errors["validation_date"] = _(
                "La data di validazione non può essere futura"
            )

        if (
            self.expected_yearly_production is not None
            and self.expected_yearly_production <= 0
        ):
            validation_errors["expected_yearly_production"] = _(
                "La produzione attesa deve essere positiva"
            )

        if self.gaudi_voltage is not None:
            if self.gaudi_voltage <= 0:
                validation_errors["gaudi_voltage"] = _(
                    "La tensione deve essere positiva"
                )
            elif self.gaudi_voltage > 500000:  # 500kV come limite massimo ragionevole
                validation_errors["gaudi_voltage"] = _(
                    "La tensione inserita supera il valore massimo consentito"
                )
...
class PlantMeasurement(models.Model):
    ...
    plant = models.ForeignKey(
        Plant, on_delete=models.CASCADE, related_name="measurements"
    )
...
class Alert(models.Model):
    ...
    type = models.CharField("Tipo", max_length=20, choices=TYPE_CHOICES, default="INFO")
    title = models.CharField("Titolo", max_length=255, default="Alert")
    message = models.TextField("Messaggio", default="")
    is_read = models.BooleanField("Letto", default=False)

    plant = models.ForeignKey(
        "Plant", on_delete=models.CASCADE, null=True, blank=True, related_name="alerts"
    )
    ...
    class Meta:
        indexes = [
            ...
            models.Index(fields=["plant", "type"]),
            models.Index(fields=["cer_configuration", "type"]),
        ]
...
class PerformanceMetrics(models.Model):
    ...
    lcp = models.FloatField(
        null=True, blank=True, help_text="Largest Contentful Paint (ms)"
    )
    ...
    fcp = models.FloatField(
        null=True, blank=True, help_text="First Contentful Paint (ms)"
    )
    ttfb = models.FloatField(null=True, blank=True, help_text="Time to First Byte (ms)")

    # Memory usage
    memory_used = models.BigIntegerField(
        null=True, blank=True, help_text="Memory used (bytes)"
    )
    memory_total = models.BigIntegerField(
        null=True, blank=True, help_text="Memory total (bytes)"
    )
    memory_limit = models.BigIntegerField(
        null=True, blank=True, help_text="Memory limit (bytes)"
    )
...
class ABTestingParticipation(models.Model):
    ...
    user_id_hash = models.CharField(
        max_length=100, help_text="Hashed user ID for consistent assignment"
    )
