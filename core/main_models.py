...
class CERMembership(models.Model):
    ...
    share_percentage = models.DecimalField("Percentuale di partecipazione", max_digits=5, decimal_places=2, default=0.00)
    ...
    def clean(self):
        ...
        if self.share_percentage > 100:
            raise ValidationError({'share_percentage': 'La percentuale non può superare il 100%'})
        if self.share_percentage < 0:
            raise ValidationError({'share_percentage': 'La percentuale non può essere negativa'})
...
class Plant(models.Model):
    ...
    code = models.CharField("Codice impianto", max_length=50, unique=True, default='PLANT001')
    pod_code = models.CharField("Codice POD", max_length=50, unique=True, blank=True, null=True)
    type = models.CharField("Tipologia", max_length=20, choices=PLANT_TYPES, default='PRODUCER')
    plant_type = models.CharField("Tipologia", max_length=20, choices=PLANT_TYPES, blank=True, null=True)
    ...
    # Dati tecnici
    power_kw = models.DecimalField("Potenza (kW)", max_digits=10, decimal_places=2, default=0.00)
    ...
    commissioning_date = models.DateField("Data di messa in servizio", blank=True, null=True)
