...
class EconomicTransactionAudit(models.Model):
    ...
    transaction_id = models.PositiveIntegerField(
        "ID Transazione", help_text="ID della transazione modificata"
    )
    ...
    operation_type = models.CharField(
        "Tipo Operazione", max_length=20, choices=OPERATION_TYPES
    )
    operation_timestamp = models.DateTimeField(
        "Timestamp Operazione", auto_now_add=True, db_index=True
    )
    ...
    user_agent = models.TextField(
        "User Agent", blank=True, help_text="Browser/applicazione utilizzata"
    )
    session_id = models.CharField(
        "ID Sessione", max_length=40, blank=True, help_text="ID della sessione"
    )

    # Motivo della modifica
    reason = models.TextField(
        "Motivo", blank=True, help_text="Motivazione della modifica"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.retention_date:
            self.retention_date = timezone.now() + timezone.timedelta(
                days=730
            )  # 2 anni
        super().save(*args, **kwargs)

    @classmethod
    def log_transaction_operation(
        cls,
        transaction,
        operation_type,
        user=None,
...
class CERDocumentAudit(models.Model):
    ...
    document_type = models.CharField(
        "Tipo Documento", max_length=20, choices=DOCUMENT_TYPES
    )
    ...
    reason = models.TextField(
        "Motivo", blank=True, help_text="Motivazione dell'operazione"
    )
...
class UserActionAudit(models.Model):
    ...
    # Dettagli azione
    action_type = models.CharField("Tipo Azione", max_length=20, choices=ACTION_TYPES)
    description = models.TextField(
        "Descrizione", help_text="Descrizione dettagliata dell'azione"
    )

    # Timestamp
    action_timestamp = models.DateTimeField(
        "Timestamp Azione", auto_now_add=True, db_index=True
    )

    # Informazioni contestuali
    ip_address = models.GenericIPAddressField("Indirizzo IP", null=True, blank=True)
    user_agent = models.TextField("User Agent", blank=True)
    session_id = models.CharField("ID Sessione", max_length=40, blank=True)
