...
from ..models.audit import EconomicTransactionAudit, CERDocumentAudit, UserActionAudit
...
def log_economic_transaction_changes(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        old_values = None  # In un'implementazione completa, salveremmo i valori precedenti
        new_values = {
            'id': str(instance.id),
            'transaction_number': instance.transaction_number,
            'status': instance.status,
...
def log_economic_transaction_deletion(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(f"Errore nel logging audit eliminazione transazione {instance.id}: {e}")
...
def log_cer_document_operation(cer_configuration, document_type, operation_type, 
                              user, document_name="", reason=""):
...
def cleanup_expired_audit_logs():
    ...
    try:
        ...
        expired_docs = CERDocumentAudit.objects.filter(
            retention_date__lt=now
        )
        ...
        expired_actions = UserActionAudit.objects.filter(
            retention_date__lt=now
        )
        actions_count = expired_actions.count()
        expired_actions.delete()
        
        logger.info(f"Pulizia audit logs completata: {economic_count} economic, {docs_count} docs, {actions_count} actions")
