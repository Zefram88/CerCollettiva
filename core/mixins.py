...
class ExcelExportMixin:
    ...
    def export_to_excel(self, request, queryset):
        ...
        try:
            ...
            # Log success
            logger.info(f"Excel export completed for {self.model._meta.label}: {queryset.count()} records")
...
class ExcelImportMixin:
    ...
    def import_from_excel(self, request, file):
        ...
        try:
            ...
            # Mostra risultati
            if success_count > 0:
                messages.success(request, f"Import completato: {success_count} record importati")
            
            if error_count > 0:
                messages.warning(request, f"Import completato con errori: {error_count} record non importati")
                for error in errors[:5]:  # Mostra solo primi 5 errori
                    messages.error(request, error)
            
            # Log results
            logger.info(f"Excel import completed for {self.model._meta.label}: {success_count} success, {error_count} errors")
...
class ReportMixin:
    ...
    def generate_report(self, request, queryset, report_type='summary'):
        ...
        try:
            ...
            logger.error(f"Report generation failed for {self.model._meta.label}: {str(e)}")
