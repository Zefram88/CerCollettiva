...
class PowerValidator(BaseValidator):
    ...
    def get_default_error_message(self):
        ...
        return _("La potenza deve essere maggiore di {min} kW").format(
            min=self.min_value
        )
    ...
    def validate(self, value, cleaned_data=None):
        ...
        if value < self.min_value:
            raise ValidationError(
                _("La potenza deve essere maggiore di {min} kW").format(
                    min=self.min_value
                )
            )
        ...
        if self.max_value and value > self.max_value:
            raise ValidationError(
                _("La potenza non può superare {max} kW").format(max=self.max_value)
            )
    ...
    def validate(self, value, cleaned_data=None):
        ...
        if self.max_size_mb:
            ...
            if value.size > max_size_bytes:
                raise ValidationError(
                    _("Il file non può superare {size}MB").format(size=self.max_size_mb)
                )
...
class DateRangeValidator(BaseValidator):
    ...
    def validate(self, value, cleaned_data=None):
        ...
        if start_date and end_date and end_date < start_date:
            raise ValidationError(
                _("La data di fine non può essere anteriore alla data di inizio")
            )
            ...
            if cleaned_data.get(condition_field) == condition_value:
                if not cleaned_data.get(field_name):
                    self.add_validation_error(
                        field_name, _("Questo campo è obbligatorio")
                    )
...
IMAGE_VALIDATOR = FileTypeValidator(
    allowed_extensions=[".jpg", ".jpeg", ".png"], max_size_mb=5
)
