...
class DashboardView(CerBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if is_global_admin:
            ...
            plants = (
                Plant.objects.filter(is_active=True)
                .select_related("cer_configuration", "owner")
                .prefetch_related("devices__measurements")
            )
        else:
        ...
        if is_global_admin or administered_cer_ids.exists():
            plants_stats = {
                ...
                "by_type": plants.values("plant_type").annotate(count=Count("id")),
                "total_power": plants.aggregate(total=Sum("nominal_power"))["total"]
                or 0,
            }
        else:
        ...
        context.update(
            {
                ...
                "cer_stats": {
                    ...
                    "active_memberships": user_memberships.filter(
                        is_active=True
                    ).count(),
                },
            }
    ...
    def get_basic_plants_stats(self, plants):
        ...
        return {
            ...
            "with_cer": plants.exclude(cer_configuration=None).count(),
            "by_type": plants.values("plant_type").annotate(count=Count("id")),
            "total_power": plants.aggregate(total=Sum("nominal_power"))["total"] or 0,
        }
    ...
    def _calculate_energy_stats(self, plants, time_threshold):
        ...
        recent_power = (
            DeviceMeasurement.objects.filter(
                plant_id__in=plant_ids, timestamp__gte=time_threshold
            ).aggregate(total=Sum("power"))["total"]
            or 0
        )
        ...
        today_energy = (
            DeviceMeasurement.objects.filter(
                plant_id__in=plant_ids, timestamp__gte=today_start
        ...
        month_energy = (
            DeviceMeasurement.objects.filter(
                plant_id__in=plant_ids, timestamp__gte=month_start
            ).aggregate(total=Sum("energy_total"))["total"]
            or 0
        )
        ...
        year_energy = (
            DeviceMeasurement.objects.filter(
                plant_id__in=plant_ids, timestamp__gte=year_start
            ).aggregate(total=Sum("energy_total"))["total"]
            or 0
        )
            ...
            if is_cer_admin:
                ...
                total_power = (
                    Plant.objects.filter(
                        cer_configuration_id__in=administered_cers, is_active=True
                    ).aggregate(total_power=Sum("nominal_power"))["total_power"]
                    or 0
                )
            else:
                # Se non è admin, ottiene solo i suoi impianti
                total_power = (
                    Plant.objects.filter(owner=user, is_active=True).aggregate(
...
class CerDashboardView(CerBaseView, StaffRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["active_cer"] = CERConfiguration.objects.filter(is_active=True).count()
    ...
    def _calculate_weekly_stats(self, start_date):
        ...
        return {
            "total_energy": measurements.aggregate(total=Sum("value"))["total"] or 0,
