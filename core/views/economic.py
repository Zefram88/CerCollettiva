...
class EconomicDashboardView(CerBaseView):
    ...
    def get_economic_stats(self, cers):
        ...
        total_income = GSEIncomeTracking.objects.filter(
            cer_configuration__in=cers, payment_status="RECEIVED"
        ...
        pending_amount = GSEIncomeTracking.objects.filter(
            cer_configuration__in=cers, payment_status__in=["EXPECTED", "DELAYED"]
        ).aggregate(total=Sum("net_amount"))["total"] or Decimal("0")
...
class CERDistributionDetailView(CerBaseView, DetailView):
    ...
    def can_access_cer(self, user, cer):
        ...
        return CERMembership.objects.filter(
            user=user, cer_configuration=cer, is_active=True
        ).exists()
    ...
    def get_context_data(self, **kwargs):
        ...
        recent_payments = GSEIncomeTracking.objects.filter(
            cer_configuration=cer, payment_status="RECEIVED"
        ...
        for payment in recent_payments:
            ...
            if distribution:
                distributions.append({"payment": payment, "distribution": distribution})
    ...
    def can_edit_config(self, user, cer):
        ...
        return CERMembership.objects.filter(
            user=user, cer_configuration=cer, role="ADMIN", is_active=True
        ).exists()
...
class EconomicReportsView(CerBaseView):
    ...
    def generate_report_data(self, cers):
        ...
        for month in range(1, 13):
            ...
            monthly_trend.append({"month": month, "income": float(month_income)})
        ...
        for cer in cers:
            ...
            cer_breakdown.append({"name": cer.name, "income": float(cer_income)})
        ...
        for payment_type, display_name in GSEIncomeTracking.PAYMENT_TYPES:
            ...
            payment_type_breakdown.append(
                {"type": display_name, "income": float(type_income)}
            )
...
def distribution_simulation_ajax(request, cer_id):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Configurazione di distribuzione non trovata"}, status=404
            )
