...
class CERListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.annotate(members_count=Count("members", distinct=True))
        ...
        context.update(
            {
                "debug": {
                    ...
                    "active_cers": CERConfiguration.objects.filter(
                        is_active=True
                    ).count(),
...
class CERPublicDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        is_member = CERMembership.objects.filter(
            user=self.request.user, cer_configuration=cer, is_active=True
        ).exists()
...
class CERDetailView(BaseCERView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_staff:
            is_member = CERMembership.objects.filter(
                user=request.user, cer_configuration=cer, is_active=True
            ).exists()
            ...
            if not is_member:
                ...
                return redirect(
                    reverse("core:cer_public_detail", kwargs={"pk": cer.pk})
                )
    ...
    def get_object(self):
        ...
        return get_object_or_404(CERConfiguration, pk=self.kwargs["pk"])
        ...
        context.update(
            {
                ...
                "members": cer.memberships.filter(is_active=True).select_related(
                    "user"
                ),
                "plants": self._get_filtered_plants(cer),
                "is_admin": self.request.user.is_staff
                or (user_membership and user_membership.role == "ADMIN"),
            }
        )
    ...
    def _calculate_cer_energy_stats(self, cer, time_threshold):
        ...
        try:
            ...
            measurements = PlantMeasurement.objects.select_related("plant").filter(
                plant__cer_configuration=cer, timestamp__gte=time_threshold
            )

            # Calcola totale produzione
            producer_total = (
                measurements.filter(plant__plant_type="PRODUCER").aggregate(
            ...
            consumer_total = (
                measurements.filter(plant__plant_type="CONSUMER").aggregate(
                    total=Sum("value")  # Usa 'value' invece di 'power'
                )["total"]
                or 0
            )
            ...
            return {
                ...
                "measurement_period": {"start": time_threshold, "end": timezone.now()},
            }
...
class CERJoinView(BaseCERView, GDPRConsentRequiredMixin):
    ...
    def get_object(self):
        return get_object_or_404(CERConfiguration, pk=self.kwargs["pk"], is_active=True)
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if CERMembership.objects.filter(
            user=request.user, cer_configuration=cer
        ).exists():
...
class MemberRegistryView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = cer.member_registry.select_related(
            "membership__user", "membership__cer_configuration"
