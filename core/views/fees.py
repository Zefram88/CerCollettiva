...
class CERFeesManagementView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            MembershipCard.objects.filter(membership__cer_configuration=cer)
            .select_related("membership__user", "membership__cer_configuration")
            .order_by("-issue_date")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        paid_amount = (
            all_cards.filter(membership_fee_paid=True).aggregate(Sum("fee_amount"))[
                "fee_amount__sum"
            ]
            or 0
        )
...
def bulk_set_fees(request, cer_pk):
    ...
    try:
        ...
        cards = MembershipCard.objects.filter(membership__cer_configuration=cer)
    ...
    def get_context_data(self, **kwargs):
        ...
        for member_type, display in [
            ...
            stats_by_type[member_type] = {
                ...
                "total_amount": cards_type.aggregate(Sum("fee_amount"))[
                    "fee_amount__sum"
                ]
                or 0,
                "paid_amount": cards_type.filter(membership_fee_paid=True).aggregate(
                    Sum("fee_amount")
                )["fee_amount__sum"]
                or 0,
            }
            stats_by_type[member_type]["pending_amount"] = (
                stats_by_type[member_type]["total_amount"]
                - stats_by_type[member_type]["paid_amount"]
            )
