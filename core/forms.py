...
class InitialSuperUserForm(ValidationMixin, UserCreationForm):
    ...
    first_name = forms.CharField(
        ...
        max_length=30,
        required=True,
        widget=forms.TextInput(attrs={"placeholder": "Mario", "class": "form-control"}),
    )
    ...
    last_name = forms.CharField(
        ...
        max_length=150,
        required=True,
        widget=forms.TextInput(attrs={"placeholder": "Rossi", "class": "form-control"}),
    )
    ...
    email = forms.EmailField(
        ...
        widget=forms.EmailInput(
            attrs={"placeholder": "admin@example.com", "class": "form-control"}
        ),
    )
    ...
    password1 = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={"placeholder": "Password sicura", "class": "form-control"}
        ),
    ...
    password2 = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={"placeholder": "Ripeti la password", "class": "form-control"}
        ),
    )
    ...
    cer_name = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": "CER Demo", "class": "form-control"}
        ),
    )
    ...
    cer_code = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": "CER001", "class": "form-control"}
        ),
    )
    ...
    def clean_cer_name(self):
        ...
        if create_demo_cer and not cer_name:
            raise ValidationError(
                "Il nome CER è obbligatorio quando si crea una CER di esempio."
            )
    ...
    def clean_cer_code(self):
        ...
        if create_demo_cer and not cer_code:
            raise ValidationError(
                "Il codice CER è obbligatorio quando si crea una CER di esempio."
            )
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Nome della CER"}
            ),
            ...
            "primary_substation": forms.TextInput(
                attrs={
                    "class": "form-control",
...
class CERMembershipForm(ValidationMixin, forms.ModelForm):
    ...
    annual_consumption = forms.IntegerField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": "3000", "min": "0"}
        ),
    ...
    class Meta:
        ...
        widgets = {
            ...
            "conformity_declaration": forms.ClearableFileInput(
                attrs={
                    "class": "form-control producer-field",
                    "accept": ".pdf",
                    "style": "display: none;",  # Nascosto inizialmente
            ...
            "gse_practice": forms.ClearableFileInput(
                attrs={
                    "class": "form-control producer-field",
                    "accept": ".pdf",
                    "style": "display: none;",  # Nascosto inizialmente
            ...
            "panels_photo": forms.ClearableFileInput(
                attrs={
                    "class": "form-control producer-field",
                    "accept": "image/*",
                    "style": "display: none;",  # Nascosto inizialmente
            ...
            "inverter_photo": forms.ClearableFileInput(
                attrs={
                    "class": "form-control producer-field",
                    "accept": "image/*",
                    "style": "display: none;",  # Nascosto inizialmente
    ...
    def clean(self):
        ...
        if not cleaned_data.get("identity_document"):
            self.add_error(
                "identity_document",
                "Il documento di identità è obbligatorio per tutti i tipi di membri",
            )

        if not cleaned_data.get("fiscal_code_document"):
            self.add_error(
                "fiscal_code_document",
                "Il codice fiscale è obbligatorio per tutti i tipi di membri",
            )
        ...
        if not pod_code and not address:
            self.add_error(
                "pod_code",
                "È necessario fornire il codice POD o l'indirizzo di fornitura",
            )
            self.add_error(
                "address",
                "È necessario fornire il codice POD o l'indirizzo di fornitura",
        ...
        if member_type in ["PRODUCER", "PROSUMER"]:
            ...
            if not cleaned_data.get("annual_production"):
                self.add_error(
                    "annual_production",
                    "La produzione annua stimata è obbligatoria per i produttori",
                )

            if not cleaned_data.get("plant_power"):
                self.add_error(
                    "plant_power",
                    "La potenza dell'impianto è obbligatoria per i produttori",
                )

            if not cleaned_data.get("installation_date"):
                self.add_error(
                    "installation_date",
                    "La data di installazione è obbligatoria per i produttori",
                )
...
class PlantForm(ValidationMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": _("Nome impianto")}
            ),
            "pod_code": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "IT001E00000000"}
            ),
            "plant_type": forms.Select(attrs={"class": "form-control"}),
            "nominal_power": forms.NumberInput(
                attrs={"class": "form-control", "placeholder": _("kW")}
            ),
            "expected_yearly_production": forms.NumberInput(
                attrs={"class": "form-control", "placeholder": _("kWh/anno")}
            ),
            "connection_voltage": forms.NumberInput(
                attrs={"class": "form-control", "placeholder": "230"}
            ),
            "address": forms.TextInput(attrs={"class": "form-control"}),
            "city": forms.TextInput(attrs={"class": "form-control"}),
            "province": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "VE"}
            ),
            "zip_code": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "30100"}
            ),
            "installation_date": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "validation_date": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "expected_operation_date": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
        }
        labels = {
        ...
        help_texts = {
            "pod_code": _(
                "Codice identificativo del punto di prelievo (14-15 caratteri)"
            ),
    ...
    def clean_pod_code(self):
        ...
        if pod_code:
            ...
            # Verifica unicità
            if (
                Plant.objects.filter(pod_code=pod_code)
    ...
    def clean_expected_yearly_production(self):
        ...
        if production and production < 0:
            raise forms.ValidationError(
                _("La produzione annua attesa non può essere negativa")
            )
        return production
...
class PlantMQTTConfigForm(forms.ModelForm):
    ...
    mqtt_broker = forms.CharField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "mqtt.example.com"}
        ),
...
class InitialGaudiUploadForm(forms.Form):
    ...
    class Meta:
        help_texts = {"gaudi_file": _("Il file deve essere in formato PDF")}
