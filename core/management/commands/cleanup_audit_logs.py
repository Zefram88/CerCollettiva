    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Errore durante la pulizia: {e}')
                )
                logger.error(f"Errore comando cleanup_audit_logs: {e}")

    def _show_expired_logs(self):
        """Mostra i log che verrebbero eliminati"""
        from core.models.audit import EconomicTransactionAudit, CERDocumentAudit, UserActionAudit
        ...
        self.stdout.write(f"Audit transazioni economiche scadute: {expired_economic.count()}")
        ...
        total = expired_economic.count() + expired_docs.count() + expired_actions.count()
