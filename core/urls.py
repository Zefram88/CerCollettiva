...
from .views.gaudi import (
    NewPlantFromGaudiView,
    PlantCreateFromGaudiView,
...
from .views.setup import InitialSetupView, setup_check_view, setup_complete_view
...
urlpatterns = [
    ...
    path(
        "cer/<int:pk>/public/", CERPublicDetailView.as_view(), name="cer_public_detail"
    ),
    path("cer/<int:pk>/join/", CERJoinView.as_view(), name="cer_join"),
    path(
        "cer/<int:cer_pk>/card/", MembershipCardView.as_view(), name="membership_card"
    ),
    path(
        "cer/<int:cer_pk>/registry/",
        MemberRegistryView.as_view(),
        name="member_registry",
    ),
    path("cer/<int:cer_pk>/fees/", CERFeesManagementView.as_view(), name="cer_fees"),
    path(
        "cer/fee/<int:pk>/",
        MembershipFeeDetailView.as_view(),
        name="membership_fee_detail",
    ),
    ...
    path(
        "plants/new-from-gaudi/",
        NewPlantFromGaudiView.as_view(),
        name="plant_new_from_gaudi",
    ),
    path(
        "plants/create-from-gaudi/",
        PlantCreateFromGaudiView.as_view(),
        name="plant_create_with_gaudi",
    ),
    path(
        "plants/<int:pk>/gaudi-update/",
        PlantGaudiUpdateView.as_view(),
        name="plant_gaudi_update",
    ),
    # Plant URLs - MQTT Configuration
    path(
        "plants/<int:pk>/mqtt/", PlantMQTTConfigView.as_view(), name="plant_mqtt_config"
    ),
    # Plant URLs - Document Management
    path(
        "plants/<int:pk>/documents/",
        PlantDocumentListView.as_view(),
        name="plant_documents",
    ),
    path(
        "plants/<int:pk>/documents/upload/",
        PlantDocumentUploadView.as_view(),
        name="plant_document_upload",
    ),
    path(
        "plants/<int:pk>/documents/<int:document_id>/delete/",
    ...
    path(
        "api/plants/<int:plant_id>/measurements/",
        plant_measurements_api,
        name="plant-measurements-api",
    ),
