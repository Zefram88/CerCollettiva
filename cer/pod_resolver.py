...
class GSEPodResolver:
    ...
    def resolve_pod(self, pod_code: str) -> PODResult:
        ...
        if not self._validate_pod_format(pod_code):
            raise ValueError(
                f"Formato POD non valido: {pod_code}. Formato atteso: ITxxxExxxxxxxx"
            )
    ...
    def _get_geographic_info_from_ac(self, ac_code: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Errore nell'ottenimento info geografiche per AC {ac_code}: {str(e)}"
            )
            raise
        ...
        try:
            ...
            if data.get("features") and len(data["features"]) > 0:
                ...
                return {
                    ...
                    "fornitore": feature["attributes"].get(
                        "RAG_SOC", "Non specificato"
                    ),
                }
            return None
    ...
    def _find_comuni_intersecting_ac(self, ac_geometry: Dict) -> List[Dict]:
        ...
        if not simplified_geometry["rings"]:
            logger.warning(
                "Geometria AC senza rings, impossibile eseguire query spaziale"
            )
            return []
