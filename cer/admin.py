...
class MemberProfileAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'user_full_name', 'user_email', 'legal_type', 
        'is_complete_display', 'created_at'
    ]
    list_filter = [
        'user__legal_type', 'created_at', 'updated_at'
    ]
    search_fields = [
        'user__first_name', 'user__last_name', 'user__email',
        'fiscal_code', 'vat_number', 'legal_name'
    ]
    ...
    fieldsets = (
        ...
        ('Dati Associazione', {
            'fields': (
                'registration_number', 'statute_date'
            ),
            'classes': ('collapse',)
        }),
        ('Dati Ente Religioso', {
            'fields': ('religious_entity_code',),
            'classes': ('collapse',)
        }),
        ('Stato Profilo', {
            'fields': ('is_complete_display',),
            'classes': ('collapse',)
        })
    )
