...
class OnboardingStep1Form(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Aggiungi help text
        self.fields["legal_type"].help_text = _(
            "Verifica il tipo di soggetto selezionato"
        )
    ...
    def clean_pod_code(self):
        ...
        if not re.match(r"^IT[A-Z0-9]+$", pod_code):
            raise ValidationError(
                _("Il codice POD può contenere solo lettere e numeri")
            )
...
class OnboardingStep3Form(forms.Form):
    ...
    has_plant = forms.BooleanField(
        ...
        help_text=_(
            "Seleziona se possiedi un impianto di produzione di energia rinnovabile"
        ),
    )
    ...
    def clean(self):
        ...
        if has_plant:
            ...
            if not cleaned_data.get("plant_power"):
                self.add_error(
                    "plant_power", _("La potenza dell'impianto è obbligatoria")
                )
            if not cleaned_data.get("plant_installation_date"):
                self.add_error(
                    "plant_installation_date",
                    _("La data di installazione è obbligatoria"),
                )
    ...
    def clean_identity_document(self):
        ...
        if file:
            ...
            if file.content_type not in allowed_types:
                raise ValidationError(
                    _("Formato file non supportato. Usa PDF, JPG o PNG")
                )
...
class ProfileCompletionForm(forms.ModelForm):
    ...
    legal_type = forms.ChoiceField(
        ...
        help_text=_(
            "Seleziona il tipo di soggetto per visualizzare i campi appropriati"
        ),
    )
    ...
    class Meta:
        ...
        widgets = {
            "fiscal_code": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "es. RSSMRA80A01H501U"}
            ),
            "phone": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "es. +39 123 456 7890"}
            ),
            "address": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Via Roma 123"}
            ),
            "city": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Milano"}
            ),
            "zip_code": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "20100"}
            ),
            "province": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "MI", "maxlength": "2"}
            ),
            "vat_number": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "12345678901"}
            ),
            "legal_name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Denominazione azienda"}
            ),
            "pec": forms.EmailInput(
                attrs={"class": "form-control", "placeholder": "azienda@pec.it"}
            ),
            "sdi_code": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "ABCDEFG",
                    "maxlength": "7",
    ...
    def clean_vat_number(self):
        ...
        if vat_number and len(vat_number) != 11:
            raise forms.ValidationError(
                _("La partita IVA deve essere di 11 caratteri numerici.")
            )
        return vat_number
    ...
    def clean_province(self):
        ...
        if province and len(province) != 2:
            raise forms.ValidationError(
                _("La provincia deve essere di 2 caratteri (es. MI, RM).")
            )
    ...
    def clean(self):
        ...
        if legal_type in ["BUSINESS", "ASSOCIATION", "CHURCH", "PUBLIC"]:
            ...
            if not cleaned_data.get("vat_number"):
                self.add_error(
                    "vat_number",
                    _("La partita IVA è obbligatoria per questo tipo di soggetto"),
                )
            if not cleaned_data.get("legal_name"):
                self.add_error(
                    "legal_name",
                    _("La denominazione è obbligatoria per questo tipo di soggetto"),
                )
            if not cleaned_data.get("pec"):
                self.add_error(
                    "pec", _("La PEC è obbligatoria per questo tipo di soggetto")
                )
