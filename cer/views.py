...
from .forms import (
    ...
    OnboardingStep4Form, OnboardingStep5Form, ProfileCompletionForm
)
...
def onboarding_wizard(request):
    ...
    try:
        ...
        messages.error(request, _('Profilo membro non trovato. Contatta l\'amministratore.'))
        return redirect('core:dashboard')
    
    # Verifica che il profilo sia completo
    if not member_profile.is_complete:
        messages.warning(request, _('Completa prima tutti i campi del profilo anagrafico.'))
        return redirect('users:profile_complete')
...
def _handle_step_get(request, step, member_profile):
    ...
    if hasattr(member_profile, 'onboarding_data'):
        form = form_class(initial=member_profile.onboarding_data.get(f'step_{step}', {}))
...
def _handle_step_post(request, step, member_profile):
    ...
    if form.is_valid():
        ...
        if step == 2:
            ...
            if not step_data.get('cer_configuration') and not step_data.get('create_new_cer'):
...
def _complete_onboarding(request, member_profile):
    ...
    try:
        ...
        if create_new_cer:
            ...
            if cer_config_id:
                cer_config = get_object_or_404(CERConfiguration, id=cer_config_id, is_active=True)
            else:
                # CER unica automatica
                cer_config = CERConfiguration.objects.filter(is_active=True).first()
                if not cer_config:
                    messages.error(request, _('Nessuna CER attiva disponibile. Contatta l\'amministratore.'))
                    return redirect('cer:onboarding_step', step=2)
        ...
        messages.success(request, _('Onboarding completato con successo! Benvenuto nella CER.'))
        return redirect('core:dashboard')
        
    except Exception as e:
        logger.error(f"Errore nel completamento onboarding per utente {user.id}: {e}")
        messages.error(request, _('Errore durante il completamento dell\'onboarding. Riprova.'))
        return redirect('cer:onboarding_wizard')
...
def _create_new_cer(request, pod_data, user):
    ...
    try:
        ...
        cer_config = CERConfiguration.objects.create(
            name=new_cer_name,
            code=cer_code,
            description=new_cer_description or f"Comunità Energetica Rinnovabile creata da {user.get_full_name()}",
            primary_substation=pod_data.get('pod_code', ''),
            is_active=True,
            # Qui potresti aggiungere altri campi basati sui dati del POD
        )
        
        logger.info(f"Nuova CER creata: {cer_config.name} (ID: {cer_config.id}) da utente {user.id}")
...
def resolve_pod(request):
    ...
    if not pod_code:
        return JsonResponse({'status': 'error', 'message': 'Codice POD richiesto'}, status=400)
    ...
    try:
        ...
        with GSEPodResolver() as resolver:
            ...
            return JsonResponse({
                ...
                'cer_info': {
                    ...
                    'description': cer_config.description if cer_config else 'Comunità Energetica Rinnovabile',
                    'primary_substation': cer_config.primary_substation if cer_config else None
                },
        ...
        if "Area Convenzionale non trovata" in error_msg:
            ...
            logger.warning(f"API GSE non disponibile per POD {pod_code}, ma codice valido - procedendo con dati limitati")
            
            # Trova CER disponibili (senza filtri geografici)
            available_cer = CERConfiguration.objects.filter(
                is_active=True
...
def profile_completion(request):
    ...
    if request.method == 'POST':
        ...
        if 'legal_type' in request.POST and len(request.POST) == 2:  # Solo legal_type + CSRF
            ...
            if legal_type and user.legal_type != legal_type:
                ...
                messages.info(request, _('Tipo di soggetto aggiornato. Compila i campi appropriati.'))
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 
                _('Profilo anagrafico completato con successo! Ora puoi procedere con la configurazione della CER.')
            )
            
            return redirect('core:dashboard')
        else:
            messages.error(request, _('Ci sono errori nel form. Controlla i campi evidenziati.'))
    else:
