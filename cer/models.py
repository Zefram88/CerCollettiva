    ...
    def clean(self):
        ...
        if self.user.legal_type == 'PRIVATE' and self.pk:
            if not self.fiscal_code:
                raise ValidationError({'fiscal_code': 'Il codice fiscale è obbligatorio per le persone fisiche.'})
            if len(self.fiscal_code) != 16:
                raise ValidationError({'fiscal_code': 'Il codice fiscale per persone fisiche deve essere di 16 caratteri.'})
        ...
        # Validazioni per aziende/associazioni (solo se il profilo è già stato inizializzato)
            ...
            if self.vat_number and len(self.vat_number) != 11:
                raise ValidationError({'vat_number': 'La Partita IVA deve essere di 11 caratteri numerici.'})
        
        # Validazioni per enti pubblici (solo se il profilo è già stato inizializzato)
        elif self.user.legal_type == 'PUBLIC' and self.pk:
            if not self.legal_name:
                raise ValidationError({'legal_name': 'La denominazione è obbligatoria per gli enti pubblici.'})
